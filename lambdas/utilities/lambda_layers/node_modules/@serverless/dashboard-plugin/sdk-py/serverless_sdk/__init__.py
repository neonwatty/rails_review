...
class SDK(object):
    ...
    def transaction(self, event, context, function_name, timeout):
        ...
        def capture_exception(exception):
            try:
                ...
                error_data["errorCulprit"] = "{} ({})".format(stack_frames[-1][2], stack_frames[-1][0])
                ...
                error_data["errorId"] = "{}!${}".format(exc_type.__name__, str(exc_value)[:200])
        ...
        context.serverless_sdk = SDK_METHOD_WRAPPER(capture_exception, tag_event, span, set_endpoint, get_transaction_id)
        ...
        def finalize():
            ...
            if os.path.exists("/proc/meminfo"):
                meminfo = {line.split(":")[0].strip(): int(line.split(":")[1].strip().split(" kB")[0]) for line in open("/proc/meminfo").readlines()}
            else:
                meminfo = {}
            ...
            tags = {
                ...
                "computeCustomFunctionVersion": os.environ.get("AWS_LAMBDA_FUNCTION_VERSION"),
                "computeCustomInvokeId": None,
                "computeCustomLogGroupName": os.environ.get("AWS_LAMBDA_LOG_GROUP_NAME"),
                "computeCustomLogStreamName": os.environ.get("AWS_LAMBDA_LOG_STREAM_NAME"),
                "computeCustomMemorySize": os.environ.get("AWS_LAMBDA_FUNCTION_MEMORY_SIZE"),
                ...
                "computeMemoryPercentageUsed": (meminfo["MemTotal"] - meminfo["MemFree"]) / meminfo["MemTotal"] if meminfo else None,
                ...
                "computeRuntime": "aws.lambda.python.{}".format(sys.version.split(" ")[0]),
    ...
    def instrument_botocore(self):
            ...
            if not self.disable_aws_spans:
                with self.span("aws") as span:
                    try:
                        ...
                        span.set_tag("requestHostname", instance._endpoint.host.split("://")[1])
                        span.set_tag(
                            "aws",
                            {
        ...
        try:
            wrapt.wrap_function_wrapper("botocore.client", "BaseClient._make_api_call", wrapper)
        except ImportError:
            pass
    ...
    def instrument_urllib3(self):
        def wrapper(wrapped, instance, args, kwargs):
            ...
            if (
                ...
                and (
                    ...
                    not user_agent.startswith("Boto3") or os.environ.get("SERVERLESS_ENTERPRISE_SPANS_CAPTURE_AWS_SDK_HTTP")
                )
                and (capture_hosts.get("*", False) or capture_hosts.get(instance.host.lower(), False))
        ...
        try:
            wrapt.wrap_function_wrapper("urllib3.connectionpool", "HTTPConnectionPool.urlopen", wrapper)
        except ImportError:
            pass
        try:
    ...
    def instrument_stdlib_urllib(self, module):
        def wrapper(wrapped, instance, args, kwargs):
            http_class, req = args
            status = None
            if (capture_hosts.get("*", False) or capture_hosts.get(req.host.lower(), False)) and not ignore_hosts.get(req.host.lower(), False):
                with self.span("http") as span:
                    try:
                        response = wrapped(*args, **kwargs)
                        return response
            ...
            try:
                ...
                def after(response):
                    try:
                        ...
                        set_endpoint(endpoint=None, http_method=request.method, http_status_code=status, meta={"mechanism": "flask-middleware"})
                    except:
                        pass
                    return response
